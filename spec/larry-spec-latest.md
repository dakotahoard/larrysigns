...
**Version 0.1.2 — Draft Specification** (completes the phonetic layer's two-handed and compound machinery; see §16 Changelog)
...
| Field | Content |
|---|---|
| `hands:` | `dh` (default), `ndh`, `both sym(mirror\|alt\|parallel)`, or `both free` (`dh{}`/`ndh{}` sub-blocks, §7.1.1) |
...
### 7.1.1 Independent hands: `both free`

When the two hands do different things — classically, a dominant hand acting on a non-dominant *base* hand (HELP, SCHOOL, MONEY) — declare `hands: both free` and give each hand its own sub-block:
...
- `dh{}` / `ndh{}` sub-blocks are legal only under `hands: both free`. When present, the articulator fields (`hs`, `orient`, `loc`, `mv`) live inside the sub-blocks; `nm:` stays at the top level (non-manuals are not per-hand).
- **Default hold:** a sub-block with no `mv:` holds its posture for the duration of the sign — the normal case for base hands.
- Both sub-blocks start together; movement is simultaneous. Sequential phasing between the hands is a segment matter (§7.1.2).
- **Global direction frame:** directions inside sub-blocks stay in the signer's global frame — `ipsi` always means the dominant side, and directions do not flip per hand. Two palms facing each other are `dh: palm(contra)` + `ndh: palm(ipsi)`.
- The dominant hand's `loc:` may reference the base hand (`ndh-palm`, `ndh-back`, `wrist-back-ndh`, …) and resolves against the `ndh` sub-block's posture.
- Battison's dominance condition applies as a lint warning: a static `ndh` base should use an unmarked handshape (`B A S C O 1 5`).

### 7.1.2 Sequential segments: `seg{}`

Lexicalized compounds and other signs with distinct sequential parts are written as ordered `seg{}` blocks:
...
- If any `seg{}` is present, all articulator content lives in segments. The top level may then carry only `hands:` (a default for segments that don't set their own) and `nm:` (spanning the whole sign).
- Each segment may set its own `hands:` — compound parts often differ (one-handed first element, two-handed second).
- Segments may contain `dh{}`/`ndh{}` sub-blocks; nothing nests deeper (no `seg` inside `seg`).
- **Timing:** a multi-segment sign is still **1 beat** by default (§8.1). The renderer applies compound reduction — compressed first element, blended internal transition, single overall production — rather than performing the parts as separate signs. Token-level suffixes (`~hold`, `~fast`) apply to the whole sign.
- Segments describe the *modern, reduced* form. Do not reconstruct a compound by pasting together its parts' citation forms — write what the hands actually do now.

### 7.1.3 Compound ancestry: `compound(...)`

An optional annotation between the sign ID and the `sign{}` block records lexical etymology:

```lry
def HOME compound(EAT, SLEEP) sign { ... }
```

This is **metadata**. Renderers ignore it; the fields and segments remain the renderable ground truth. The referenced IDs are etymological and need not resolve against any active dictionary — the modern form may have diverged arbitrarily far (HOME retains nothing of SLEEP but a location). A future version may define renderable pure-compound definitions, but that would require normative compound-reduction rules — exactly the kind of linguistic claim this project does not ship without Deaf review (§14). Until then, a definition without articulator content is invalid.
...
Default: 1 sign token = 1b; `ix`/`poss` = 0.5b; fingerspelling = 0.4b/letter (`~lexical` compresses to ~1.5b total); `num()` = 0.5b per numeral morpheme (profile-defined); buoy raise = 1b, `release` = 0.5b; `|` = +0.5b; `||` = +1b. Suffixes scale: `~fast` ×0.7, `~slow` ×1.5, `~hold(nb)` appends. These are *normative ratios*; absolute duration comes from `%tempo` and renderer prosody (§9.2). A multi-segment sign (§7.1.2) is still 1b by default; how the beat divides across segments is renderer discretion (compound-reduction prosody).
...
definition  = "def", SIGNID, [ compoundref ], signblock ;
compoundref = "compound", "(", SIGNID, { ",", SIGNID }, ")" ;
              (* non-normative ancestry metadata, §7.1.3 *)
signblock   = "sign", "{", ( { field | handblock } | { segblock } ), "}" ;
segblock    = "seg", "{", { field | handblock }, "}" ;
handblock   = ( "dh" | "ndh" ), "{", { field }, "}" ;
clblock     = "cl", "{", { field }, "}" ;
...
4. **Closed fieldkey set.** Phonetic-block fieldkeys are frozen per spec version (`hands hs orient loc mv nm`); a field's value list ends at the next `fieldkey :`, at a sub-block opener (`dh {`, `ndh {`, `seg {` — §7.1.1–7.1.2), or at `}`. Sub-block openers are distinguished from fields by the brace where a field has `:` (two tokens of lookahead, same budget as rule 3). Profiles may extend inventories and bundles but may **not** add fieldkeys — that would make field boundaries unparseable for older tools.
5. **Digit-initial handshape names.** The `asl.v1` inventory contains handshape identifiers that begin with a digit (`1-bent`, `5-lax`, §10.1). These must be matched before the number rule; a standard tokenizer otherwise lexes `1-bent` as `1` + `-bent` and fails. The pattern is digits, then a hyphen, then a letter — the mandatory hyphen keeps plain numerals (`3`), durations (`3b`), and degrees (`120deg`) in the number rule.
...
**0.1.2** — Completes the phonetic layer's two-handed and compound machinery, prompted by drafting the first 100-entry core dictionary: `hands: both free` fully specified with `dh{}`/`ndh{}` sub-blocks, default-hold for base hands, and a global direction frame (§7.1.1); sequential `seg{}` segments for lexicalized compounds with 1-beat compound-reduction timing (§7.1.2, §8.1); `compound(...)` ancestry annotation as non-normative metadata (§7.1.3); EBNF extended for all three (§13); lexical rule 4 amended (field values also terminate at sub-block openers) and rule 5 added (digit-initial handshape names) (§13.1).
